    ...
    return (
      <div key={this.props.contact.key} className='FullContactCard' role="article">
        ...
        <div className="ContactActionButtons">
          ...
          <div>
            <input type="button" className="ContactButton" onClick={() => this.props.handleDeselect()} value="^" />
            <input type="button" className="ContactButton" onClick={() => this.props.reference.remove()} value="X" />
          </div>
        </div>
      </div>
    );
